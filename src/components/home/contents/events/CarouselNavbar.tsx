...
const CarouselNavbar = ({ items }: { items: TEvents }) => {
  ...
  return (
    <div
      className={cn(
        'container flex flex-col rounded-md bg-kt-white p-2.5 opacity-70 transition-opacity duration-300 hover:border-b-4 hover:opacity-100',
        kategorieBorderStyle[kategorie],
      )}
    >
      <div
        className={cn(
          'mb-2.5 inline-block max-w-fit rounded-lg p-1 text-xs font-bold text-white',
